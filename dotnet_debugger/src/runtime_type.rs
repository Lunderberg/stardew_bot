...
impl RuntimeType {
    pub fn parse(&self, bytes: &[u8]) -> Result<RuntimeValue, Error> {
        match self {
            ...
            RuntimeType::Unknown => {
                Err(Error::RequiredKnownType("RuntimeType.parse"))
            }
        }
    }
    ...
    pub fn size_bytes(&self) -> Result<usize, Error> {
        match self {
            ...
            RuntimeType::Unknown => {
                Err(Error::RequiredKnownType("RuntimeType.size_bytes"))
            }
        }
    }
    ...
    /// known element type.
    ///
    /// This is used when caching type information, to ensure that the
    /// cache is only used when the type information is known.
    pub(crate) fn is_complete(&self) -> bool {
        match self {
            RuntimeType::Unknown => false,

            RuntimeType::Prim(_) | RuntimeType::DotNet(DotNetType::String) => {
                true
            }
