...
impl BotGoal for MineDelvingGoal {
    ...
    fn apply(
        ...
        println!("Num Objects: {}", current_room.objects.len());
        current_room.objects.iter().for_each(|obj| {
            println!("At {}, there's a {}", obj.tile, obj.kind)
        });

        println!("Num Action tiles: {}", current_room.action_tiles.len());
        current_room.action_tiles.iter().for_each(|(tile, action)| {
            println!("At {tile}, action = '{action}'")
        });

        println!("Num buildings: {}", current_room.buildings.len());
        current_room.buildings.iter().for_each(|building| {
            println!(
                "At {}, building kind = '{}'",
                building.shape, building.kind
            )
        });
